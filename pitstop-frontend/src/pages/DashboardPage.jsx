  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: "var(--bg)" }}>
      <style>{`
        ...
        .ps-r { position:absolute; border-radius:50%; pointer-events:none; animation: psRing 3.2s ease-out infinite; }
        .ps-r1 { animation-delay:0s; } .ps-r2 { animation-delay:0.8s; }
        .ps-r3 { animation-delay:1.6s; } .ps-r4 { animation-delay:2.4s; }
      `}</style>
      <div style={{ position:"absolute", inset:0, opacity:0.07, backgroundImage:"linear-gradient(var(--gold) 1px,transparent 1px),linear-gradient(90deg,var(--gold) 1px,transparent 1px)", backgroundSize:"24px 24px" }} />
      <div style={{ position:"absolute", inset:0, pointerEvents:"none", background:"radial-gradient(ellipse 75% 55% at 50% 43%, transparent 25%, rgba(10,10,15,0.75) 100%)" }} />
      {[
      ...
      <div style={{ position:"absolute", top:"43%", left:"50%", transform:"translate(-50%,-50%)", width:340, height:340, borderRadius:"50%", border:"1px solid rgba(255,183,0,0.08)", pointerEvents:"none" }} />
      ...
      <div style={{ position:"absolute", top:78, right:16, background:"rgba(17,17,24,0.88)", border:"1px solid rgba(74,222,128,0.25)", borderRadius:12, padding:"8px 12px", backdropFilter:"blur(6px)", textAlign:"center" }}>
...
function ActiveMap({ bottomOffset }) {
  return (
    <div style={{ position:"absolute", inset:0, background:"var(--bg)" }}>
      ...
      <div style={{ position:"absolute", top:"22%", left:"28%", background:"rgba(17,17,24,0.9)", border:"0.5px solid var(--border)", borderRadius:8, padding:"5px 10px", fontSize:11, color:"var(--gold)", fontWeight:600, backdropFilter:"blur(4px)" }}>~8 mins away</div>
      <div style={{ position:"absolute", left:16, bottom:bottomOffset+14, display:"flex", alignItems:"center", gap:5, transition:"bottom 0.35s cubic-bezier(0.32,0.72,0,1)" }}>
        <div style={{ width:5, height:5, borderRadius:"50%", background:"var(--green)" }} />
        <span style={{ fontSize:10, color:"rgba(74,222,128,0.6)", letterSpacing:"0.07em", fontVariant:"all-small-caps" }}>Your location</span>
      </div>
...
export default function DashboardPage() {
  ...
  return (
    ...
    >
      {/* Map */}
      {hasActiveJob ? <ActiveMap bottomOffset={mapBottomOffset} /> : <HeartbeatMap bottomOffset={mapBottomOffset} />}

      {/* ── TopBar — absolute over map with gradient fade ── */}
      <div style={{
        ...
        <TopBar
          centerContent={
            <div style={{ background:"rgba(255,255,255,0.06)", border:"0.5px solid var(--border)", borderRadius:20, padding:"5px 12px", display:"flex", alignItems:"center", gap:5 }}>
      ...
      >
        ...
        {!hasActiveJob && (
          <>
            ...
            {history.length > 0 ? (
              <>
                <div style={{ fontSize:10, color:"var(--text-3)", letterSpacing:"1px", textTransform:"uppercase", fontWeight:600, margin:"12px 0 8px" }}>
                  Recent request
                </div>
                <JobCard job={history[0]} onClick={() => navigate("/history")} />
              </>
            ) : (
            ...
            {/* Expanded: second job */}
            {sheetExpanded && history.length > 1 && (
              <>
                <div style={{ height:8 }} />
                <JobCard job={history[1]} onClick={() => navigate("/history")} />
              </>
            )}
          </>
        )}
        ...
        {hasActiveJob && (
          <div style={{ background:"var(--surface)", border:"1px solid rgba(255,183,0,0.3)", borderRadius:16, padding:14 }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
              <div>
                <div style={{ fontSize:15, fontWeight:600, color:"var(--text)" }}>{activeJob.problemType?.replace(/_/g," ")}</div>
